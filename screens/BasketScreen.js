...
const BasketScreen = () => {
    ...
    useMemo(() => {
        const groupedItems = items.reduce((results, item) => {
            (results[item.id] = results[item.id] || []).push(item); //creates an object with an id and a list of the corresponding dish
            return results;
        }, {})

        setGroupedItemsInBasket(groupedItems);
    }, [items])//if value of items doesnt change it won't recalculate it
